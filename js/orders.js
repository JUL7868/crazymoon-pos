window.removeItem = async function removeItem(item_id) {
  ...
  try {
    const res = await fetch(API.orders, {
      ...
      body: JSON.stringify({ action: 'remove_item', item_id, order_id: tab.order_id }),
    });
...
window.updateQty = async function updateQty(item_id, order_id, qty) {
  ...
  try {
    const res = await fetch(API.orders, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'update_qty', item_id, order_id, qty }),
    });
...
window.pushItem = async function pushItem(menu_item_id, name, category, size, unit_price, notes = '') {
  ...
  if (existing) {
    await updateQty(existing.id, tab.order_id, existing.qty + 1);
    return;
  }
